...
class API:
    def __init__(self):
        ...
        # Setup secure CORS: ONLY allow your real frontend (Railway) for /api/*
        frontend_origin = os.getenv(
            "FRONTEND_ORIGIN", "https://frontend-production-aeb1.up.railway.app"
        )
        CORS(
            self.app,
            resources={
    ...
    def run(self):
        ...
        for rule in self.app.url_map.iter_rules():
            print(f" - {rule}")
        self.app.run(host="0.0.0.0", port=8000, debug=True)  # Be explicit for cloud
